...
static void meshNekParallelConnectNodes(mesh_t* mesh, bool numberInterior)
{
  ...
  hlong ngv = nek::set_glo_num(mesh->N + 1, mesh->cht, numberInterior);
   ...
   for (int e = 0; e < mesh->Nelements; e++) {
      for (int j = 0; j < mesh->Nq; j++) {
        for (int i = 0; i < mesh->Nq; i++) {
          ...
          const dlong idx5 = e * meshExt->Np + (meshExt->Nq-1) * meshExt->Nfp + (j+1) * meshExt->Nq + (i+1);
      ...
      for (int k = 0; k < mesh->Nq; k++) {
        for (int i = 0; i < mesh->Nq; i++) {
            ...
            const dlong idx3 = e * meshExt->Np + (k+1) * meshExt->Nfp + (meshExt->Nq-1) * meshExt->Nq + (i+1);
      ...
      for (int k = 0; k < mesh->Nq; k++) {
        for (int j = 0; j < mesh->Nq; j++) {
            ...
            const dlong idx2 = e * meshExt->Np + (k+1) * meshExt->Nfp + (j+1) * meshExt->Nq + (meshExt->Nq-1);
