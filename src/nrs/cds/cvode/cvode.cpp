...
cvode_t::cvode_t(cds_t *_cds)
{
  ...
  o_U0 = platform->device.malloc<dfloat>(cds->nEXT * cds->NVfields * fieldOffset);

  if (platform->options.compareArgs("MOVING MESH", "TRUE")) {
    o_meshU0 = platform->device.malloc<dfloat>(cds->nEXT * cds->NVfields * fieldOffset);
    o_xyz0 = platform->device.malloc<dfloat>(cds->NVfields * fieldOffset);
  }
  ...
  this->gsh = oogs::setup(cds->meshV->ogs, this->Nscalar, fieldOffset, ogsDfloat, NULL, OOGS_AUTO);
...
int cvode_t::cvodeErrorWt(N_Vector y, N_Vector ewt)
{
  ...
  const auto cvodeCHT = cds->cht && cds->cvodeSolve[0];
  if (cvodeCHT) {
...
void cvode_t::setupDirichletMask()
{
  ...
  int NelemV = cds->meshV->Nelements;
  ...
  o_maskValues = platform->device.malloc<dfloat>(cds->nEXT * maskOffset);
}

void cvode_t::applyDirichlet(double time)
{
  // extrapolate masked Dirichlet values to current time state
  if (this->isRhsEvaluation() && (this->externalTStep > cds->nEXT)) {

    if (this->Nmasked == 0) {
      return;
    }

    const int extOrder = std::min(this->externalTStep, cds->nEXT);
    this->extrapolateDirichletKernel(this->Nmasked,
                                     ...
                                     o_coeffExt,
                                     this->o_maskValues,
                                     cds->o_S + this->minCvodeScalarId * fieldOffset);
    return;
  }
...
void cvode_t::defaultRHS(double time, double t0, const LVector_t<dfloat> &o_y, LVector_t<dfloat> &o_ydot)
{
  ...
  if (time != tprev) {
    ...
    const int bdfOrder = std::min(this->externalTStep, cds->nBDF);
    const int extOrder = std::min(this->externalTStep, cds->nEXT);
    nek::extCoeff(_coeffEXT.data(), dtCvode.data(), extOrder, bdfOrder);
    nek::bdfCoeff(&this->_g0, _coeffBDF.data(), dtCvode.data(), bdfOrder);
    for (int i = cds->nEXT; i > extOrder; i--) {
      _coeffEXT[i - 1] = 0.0;
    }
    for (int i = cds->nBDF; i > bdfOrder; i--) {
      _coeffBDF[i - 1] = 0.0;
    }
    ...
    if (movingMesh) {
      ...
      // restore prior to integration in move
      {
        ...
        mesh->o_U.copyFrom(o_meshU0, cds->NVfields * fieldOffset);
      }
  ...
  const bool chtCVODE = cds->cht && cds->cvodeSolve[0];
  ...
  auto applyOgsOperation = [&](auto ogsFunc) {
    if (chtCVODE) {
      ogsFunc(cds->o_NLT, 1, cds->fieldOffset[0], ogsDfloat, ogsAdd, cds->gshT);
    }
    ...
    if (!chtCVODE || (chtCVODE && this->Nscalar > 1)) {
      ...
      auto o_fld = cds->o_NLT + cds->fieldOffsetScan[startScalar];
  ...
  if (userLocalPointSourceE) {
    ...
    deviceMemory<dfloat> o_S(cds->o_S);
  ...
  // weight FS by inverse assembled mass matrix, add pointSource and finally divide by "rho"
  {
    ...
    auto o_FS = cds->o_NLT + cds->fieldOffsetScan[startScalar];
    ...
    this->fusedAddRhoDivKernel(cds->mesh[0]->Nlocal,
                               ...
                               o_rho,
                               o_ptSource,
                               (addDpdtTerm) ? (*cds->dp0thdt) * (*cds->alpha0Ref) : static_cast<dfloat>(0.0),
                               o_FS);
...
void cvode_t::solve(double t0, double t1, int tstep)
{
  ...
  auto mesh = (cds->cht) ? cds->mesh[0] : cds->meshV;
  ...
  for (int s = cds->nEXT; s > 1; s--) {
    const auto N = cds->NVfields * fieldOffset;
    o_U0.copyFrom(o_U0, N, (s - 1) * N, (s - 2) * N);
  }
  o_U0.copyFrom(cds->o_U, cds->NVfields * fieldOffset);
  ...
  if (movingMesh) {
    ...
    o_meshU0.copyFrom(mesh->o_U, cds->NVfields * fieldOffset);
  ...
  cds->o_U.copyFrom(o_U0, cds->NVfields * fieldOffset);
...
void cvode_t::printInfo(bool printVerboseInfo)
{
  ...
  if (printVerboseInfo) {
    printf("%-10s: nsteps %03ld  nRHS %03ld  nni %01ld  nni/nsteps %.1f  nli %03ld  nli/nni %.1f\n",
           scalars.c_str(),
           nsteps,
           nrhs,
           nni,
