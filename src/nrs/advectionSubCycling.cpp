...
static dfloat *extCoeffs(int nEXT, double time, dfloat tstage, dfloat sdt, dfloat *dt, dfloat *nodesRK, int rk)
{
                 ...
                 occa::memory &o_u0)
{
  ...
  if(! o_weights.isInitialized()) {
    o_weights = platform->device.malloc<dfloat>(nRK, weights);
  }
  ...
  for (int rk = 0; rk < nRK; ++rk) {
      ...
      if (movingMesh) {
        nStagesSum3Kernel(meshV->Nlocal,
                          meshOffset,
                          nEXT,
                          extC[0],
                          extC[1],
...
occa::memory advectionSubcyclingRK(mesh_t *_mesh, mesh_t *_meshV,
                                   double time, dfloat *dt, int Nsubsteps, dfloat *coeffBDF, int nEXT, 
                                   int nFields, occa::kernel _opKernel, oogs_t *_gsh, 
                                   dlong _meshOffset, dlong _fieldOffset, dlong _cubatureOffset, dlong fieldOffsetSum,
                                   occa::memory o_Urst, occa::memory o_U)
{
  ...
  subCycleInitU0Kernel = platform->kernels.get("nrs-subCycleInitU0");
  nStagesSum3Kernel = platform->kernels.get("core-nStagesSum3");
  subCycleRKKernel = platform->kernels.get("nrs-subCycleRK");
