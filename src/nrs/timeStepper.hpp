...
occa::memory velocityStrongSubCycle(nrs_t* nrs, int nEXT, double time, occa::memory o_U);
occa::memory velocityStrongSubCycleMovingMesh(nrs_t* nrs, int nEXT, double time, occa::memory o_U);
void fluidSolve(nrs_t* nrs, double time, occa::memory o_P, occa::memory o_U, int stage, int tstep);

void makeq(nrs_t *nrs, double time, int tstep, occa::memory o_FS,
           occa::memory o_BF);
occa::memory scalarStrongSubCycleMovingMesh(cds_t *cds, int nEXT, double time,
                                            int is, occa::memory o_U,
                                            occa::memory o_S);
occa::memory scalarStrongSubCycle(cds_t *cds, int nEXT, double time, int is,
