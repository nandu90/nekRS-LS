#include "nrs.hpp"

// lower than any other possible Dirichlet value
static constexpr dfloat TINY = -1e30;

void nrs_t::applyZeroNormalMask(mesh_t *mesh,
                                dlong Nelements,
                                const occa::memory &o_elementList,
                                const occa::memory &o_EToB,
                                const occa::memory &o_mask,
                                occa::memory &o_x)
{
  if (Nelements == 0) {
    return;
  }

  applyZeroNormalMaskKernel(Nelements,
                            fieldOffset,
                            o_elementList,
                            mesh->o_sgeo,
                            o_mask,
                            mesh->o_vmapM,
                            o_EToB,
                            o_x);
}

void nrs_t::applyZeroNormalMask(mesh_t *mesh,
                                const occa::memory &o_EToB,
                                const occa::memory &o_mask,
                                occa::memory &o_x)
{
  applyZeroNormalMaskKernel(mesh->Nelements,
                            fieldOffset,
                            mesh->o_elementList,
                            mesh->o_sgeo,
                            o_mask,
                            mesh->o_vmapM,
                            o_EToB,
                            o_x);
}

void nrs_t::applyDirichletVelocity(double time, occa::memory &o_U, occa::memory &o_Ue, occa::memory &o_P)
{
  if (bc.hasUnalignedMixed("velocity")) {
    applyZeroNormalMask(mesh, uvwSolver->o_EToB(), o_zeroNormalMaskVelocity, o_U);
    applyZeroNormalMask(mesh, uvwSolver->o_EToB(), o_zeroNormalMaskVelocity, o_Ue);
  }

  const auto neknekFieldOffset = neknek ? neknek->fieldOffset() : 0;

  occa::memory o_tmp = platform->deviceMemoryPool.reserve<dfloat>((NVfields + 1) * fieldOffset);
  platform->linAlg->fill((1 + NVfields) * fieldOffset, TINY, o_tmp);

  for (int sweep = 0; sweep < 2; sweep++) {
    pressureDirichletBCKernel(mesh->Nelements,
                              time,
                              fieldOffset,
                              mesh->o_sgeo,
                              mesh->o_x,
                              mesh->o_y,
                              mesh->o_z,
                              mesh->o_vmapM,
                              mesh->o_EToB,
                              o_EToB,
                              o_rho,
                              o_mue,
                              o_usrwrk,
                              o_Ue,
                              o_tmp);

    velocityDirichletBCKernel(mesh->Nelements,
                              fieldOffset,
                              time,
                              mesh->o_sgeo,
                              o_zeroNormalMaskVelocity,
                              mesh->o_x,
                              mesh->o_y,
                              mesh->o_z,
                              mesh->o_vmapM,
                              mesh->o_EToB,
                              o_EToB,
                              o_rho,
                              o_mue,
                              neknekFieldOffset,
                              neknek ? neknek->o_pointMap() : o_NULL,
                              neknek ? neknek->o_U() : o_NULL,
                              o_usrwrk,
                              o_U,
                              o_tmp.slice(fieldOffset));

    oogs::startFinish(o_tmp, 1 + NVfields, fieldOffset, ogsDfloat, (sweep == 0) ? ogsMax : ogsMin, gsh);
  }

  if (pSolver->Nmasked()) {
    auto o_dirichlet = o_tmp.slice(0, fieldOffset);
    maskCopyKernel(pSolver->Nmasked(), 0, 0, pSolver->o_maskIds(), o_dirichlet, o_P);
  }

  auto o_UDirichlet = o_tmp.slice(fieldOffset);
  if (uvwSolver) {
    if (uvwSolver->Nmasked()) {
      maskCopy2Kernel(uvwSolver->Nmasked(),
                      0 * fieldOffset,
                      0 * fieldOffset,
                      uvwSolver->o_maskIds(),
                      o_UDirichlet,
                      o_U,
                      o_Ue);
    }
  } else {
    int cnt = 0;
    for (auto &solver : {uSolver, vSolver, wSolver}) {
      if (solver->Nmasked()) {
        maskCopy2Kernel(solver->Nmasked(),
                        cnt * fieldOffset,
                        cnt * fieldOffset,
                        solver->o_maskIds(),
                        o_UDirichlet,
                        o_U,
                        o_Ue);
      }
      cnt++;
    }
  }
}

void nrs_t::applyDirichletScalars(double time, occa::memory &o_S, occa::memory &o_Se)
{
  const auto neknekFieldOffset = neknek ? neknek->fieldOffset() : 0;
  for (int is = 0; is < cds->NSfields; is++) {
    if (!cds->compute[is]) {
      continue;
    }
    if (cds->cvodeSolve[is]) {
      continue;
    }
    mesh_t *mesh = cds->mesh[0];
    oogs_t *gsh = cds->gshT;
    if (is) {
      mesh = cds->meshV;
      gsh = cds->gsh;
    }

    auto o_diff_i = cds->o_diff + cds->fieldOffsetScan[is];
    auto o_rho_i = cds->o_rho + cds->fieldOffsetScan[is];

    occa::memory o_SiDirichlet = platform->deviceMemoryPool.reserve<dfloat>(cds->fieldOffset[is]);
    platform->linAlg->fill(cds->fieldOffset[is], TINY, o_SiDirichlet);

    for (int sweep = 0; sweep < 2; sweep++) {
      cds->dirichletBCKernel(mesh->Nelements,
                             cds->fieldOffset[is],
                             is,
                             time,
                             mesh->o_sgeo,
                             mesh->o_x,
                             mesh->o_y,
                             mesh->o_z,
                             mesh->o_vmapM,
                             mesh->o_EToB,
                             cds->o_EToB + is * cds->EToBOffset,
                             cds->o_Ue,
                             o_diff_i,
                             o_rho_i,
                             neknekFieldOffset,
                             neknek ? neknek->o_pointMap() : o_NULL,
                             neknek ? static_cast<int>(neknek->o_U().isInitialized()) : 0,
                             neknek ? neknek->o_U() : o_NULL,
                             neknek ? neknek->o_S() : o_NULL,
                             neknek ? neknek->o_scalarIndices() : o_NULL,
                             *(cds->o_usrwrk),
                             o_SiDirichlet);

      oogs::startFinish(o_SiDirichlet,
                        1,
                        cds->fieldOffset[is],
                        ogsDfloat,
                        (sweep == 0) ? ogsMax : ogsMin,
                        gsh);
    }
    occa::memory o_Si = o_S.slice(cds->fieldOffsetScan[is], cds->fieldOffset[is]);

    if (o_Se.isInitialized()) {
      occa::memory o_Si_e = o_Se.slice(cds->fieldOffsetScan[is], cds->fieldOffset[is]);

      if (cds->solver[is]->Nmasked()) {
        cds->maskCopy2Kernel(cds->solver[is]->Nmasked(),
                             0,
                             0,
                             cds->solver[is]->o_maskIds(),
                             o_SiDirichlet,
                             o_Si,
                             o_Si_e);
      }
    } else {
      if (cds->solver[is]->Nmasked()) {
        cds->maskCopyKernel(cds->solver[is]->Nmasked(),
                            0,
                            0,
                            cds->solver[is]->o_maskIds(),
                            o_SiDirichlet,
                            o_Si);
      }
    }
  }
}

void nrs_t::applyDirichletMesh(double time, occa::memory &o_UM, occa::memory &o_UMe, occa::memory &o_U)
{
  auto mesh = (cht) ? cds->mesh[0] : this->mesh;
  if (bc.hasUnalignedMixed("mesh")) {
    applyZeroNormalMask(mesh, meshSolver->o_EToB(), o_zeroNormalMaskMeshVelocity, o_UM);
    applyZeroNormalMask(mesh, meshSolver->o_EToB(), o_zeroNormalMaskMeshVelocity, o_UMe);
  }

  occa::memory o_UDirichlet = platform->deviceMemoryPool.reserve<dfloat>(NVfields * fieldOffset);
  platform->linAlg->fill(NVfields * fieldOffset, TINY, o_UDirichlet);

  for (int sweep = 0; sweep < 2; sweep++) {
    mesh->velocityDirichletKernel(mesh->Nelements,
                                  fieldOffset,
                                  time,
                                  static_cast<int>(bc.useDerivedMeshBoundaryConditions()),
                                  mesh->o_sgeo,
                                  o_zeroNormalMaskMeshVelocity,
                                  mesh->o_x,
                                  mesh->o_y,
                                  mesh->o_z,
                                  mesh->o_vmapM,
                                  mesh->o_EToB,
                                  o_EToBMeshVelocity,
                                  o_meshRho,
                                  o_meshMue,
                                  o_usrwrk,
                                  o_U,
                                  o_UDirichlet);

    oogs::startFinish(o_UDirichlet,
                      NVfields,
                      fieldOffset,
                      ogsDfloat,
                      (sweep == 0) ? ogsMax : ogsMin,
                      gshMesh);
  }

  if (meshSolver->Nmasked()) {
    maskCopy2Kernel(meshSolver->Nmasked(),
                    0 * fieldOffset,
                    0 * fieldOffset,
                    meshSolver->o_maskIds(),
                    o_UDirichlet,
                    o_UM,
                    o_UMe);
  }
}
