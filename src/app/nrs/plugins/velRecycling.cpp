...
static void _setup(occa::memory &o_wrk_, const int bID_, const dfloat wbar_)
{
  ...
  auto &fluid = dynamic_cast<nrs_t *>(platform->solver)->fluid;
  o_wrk = o_wrk_;
  bID = bID_;
  wbar = wbar_;
  ...
  nekrsCheck(o_wrk.length() < mesh->dim * fluid->fieldOffset,
...
void velRecycling::copy()
{
  ...
  if (interp) {
    const dlong offset = o_Uint.length() / mesh->dim;
    interp->eval(mesh->dim, fluid->fieldOffset, fluid->o_U, offset, o_Uint);
    maskCopyKernel(interp->numPoints(), offset, fluid->fieldOffset, o_maskIds, o_Uint, o_wrk);
  } else {
    o_wrk.copyFrom(fluid->o_U, mesh->dim * fluid->fieldOffset);
    ...
    setBCVectorValueKernel(mesh->Nelements,
                           ...
                           mesh->o_vmapM,
                           mesh->o_EToB);
    oogs::startFinish(o_wrk, mesh->dim, fluid->fieldOffset, ogsDfloat, ogsAdd, ogs);
  }

  auto flux = mesh->surfaceAreaNormalMultiplyIntegrate(fluid->fieldOffset, o_bID.length(), o_bID, o_wrk);
  ...
  platform->linAlg->scale(mesh->dim * fluid->fieldOffset, scale, o_wrk);
}

void velRecycling::setup(occa::memory o_wrk_, const hlong eOffset, const int bID_, const dfloat wbar_)
{
                         ...
                         const dfloat wbar_)
{
  ...
  o_Uint = platform->device.malloc<dfloat>(mesh->dim * alignStride<dfloat>(nPoints));
