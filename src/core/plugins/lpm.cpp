                                ...
                                bool output)
{
  ...
  if (interpFieldIds.count(interpFieldName) == 0) {
    ...
    offsetFieldsInterp[interpFieldName] = fieldOffset;
...
void lpm_t::initialize(int nParticles, double t0, const occa::memory &o_y0)
{
  ...
  for (auto [fieldName, nFields] : interpFieldCounts) {
    ...
    laggedInterpFields[fieldName] = platform->device.malloc<dfloat>(nEXT * nFields * offset);
    extrapolatedInterpFields[fieldName] = platform->device.malloc<dfloat>(nFields * offset);
  }
...
void lpm_t::interpolate(const std::string &interpFieldName)
{
  ...
  interp->eval(Nfields, offset, d_fld, fieldOffset_, d_interpFld);
  if (timerLevel != TimerLevel::None) {
...
void lpm_t::integrate(double tf)
{
  ...
  if (time >= tf) {
    for (auto [fieldName, o_field] : laggedInterpFields) {
      ...
      o_field.copyFrom(o_currentField, Nfields * offset);
    }
  ...
  for (auto [fieldName, o_field] : laggedInterpFields) {
    ...
    o_extField.copyFrom(o_field, Nfields * offset);
  }
  ...
  if (userODESolver_) {
    ...
    userODESolver_(this, time, tf, tstep, o_y_, userdata_, o_ydot_);
  } else {
...
void lpm_t::extrapolateFluidState(dfloat tEXT)
{
  ...
  std::copy(dtOuter, dtOuter + extOrder, dtEXT.begin());
  dtEXT[0] = tEXT - time;

  nek::extCoeff(coeffEXT.data(), dtEXT.data(), extOrder, extOrder);
  ...
  for (auto [fieldName, o_field] : laggedInterpFields) {
    ...
    auto o_extField = extrapolatedInterpFields.at(fieldName);
    const auto offset = offsetFieldsInterp[fieldName];
    launchKernel("core-extrapolate", mesh->Nlocal, Nfields, nEXT, offset, o_coeffEXT, o_field, o_extField);
  }
}
...
void lpm_t::registerKernels(occa::properties &kernelInfo)
{
  ...
  fileName = oklpath + "/core/plugins/" + "lpm.okl";
