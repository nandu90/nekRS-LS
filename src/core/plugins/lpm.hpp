...
public:
  ...
  using rhsFunc_t = std::function<void(lpm_t *lpm,
                                       double time,
                                       const deviceMemory<dfloat>& o_y,
                                       void *userdata,
                                       deviceMemory<dfloat>& o_ydot)>;

  using odeSolverFunc_t = std::function<void(lpm_t *lpm,
                                             double t0,
                                             double tf,
                                             int step,
                                             const deviceMemory<dfloat>& o_y,
  ...
  int nInterpFields() const
  {
    return nInterpFields_;
  }
...
private:
  ...
  std::map<std::string, dlong> offsetFieldsInterp;
