...
void planarCopy::_setup(mesh_t * mesh_, const occa::memory &o_Uin_, const int nFields_, const dlong fieldOffset_, const int bID_, occa::memory& o_out_)
{
  mesh = mesh_;
...
void planarCopy::execute()
{
  ...
  if (interp) {
    ...
    if (!kernel.isInitialized()) kernel = platform->kernelRequests.load("planarCopy::", "maskCopy");
    ...
    static occa::kernel kernel;
    if (!kernel.isInitialized()) kernel = platform->kernelRequests.load("planarCopy::", "setBCValue");
                     ...
                     occa::memory& o_out_)
{
  ...
  const auto verbosity = platform->verbose() ? pointInterpolation_t::VerbosityLevel::Detailed : pointInterpolation_t::VerbosityLevel::Basic;
