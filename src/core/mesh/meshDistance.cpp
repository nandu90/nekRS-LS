...
occa::memory
mesh_t::distance(int nbID, const occa::memory &o_bID, dlong offsetFld, std::string type_, int maxIter)
{
  auto type = lowerCase(type_);
...
occa::memory mesh_t::minDistance(int nbID, const occa::memory &o_bID, std::string type_, int maxIter)
{
  auto type = lowerCase(type_);
