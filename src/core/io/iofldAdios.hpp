...
private:
  ...
  void convertField(const std::vector<occa::memory> &o_fld, occa::memory& fldConv);
  ...
  template <typename T> 
  void putVariable(const std::string& name, const occa::memory& data,
                        adios2::Dims dim, adios2::Mode putMode = adios2::Mode::Deferred)
  {
  ...
  void putVariable(std::string name, const variantType& variant, adios2::Mode putMode = adios2::Mode::Deferred)
  {
    std::visit([this, &name](const auto& variant)
    {
      if constexpr (std::is_same_v< std::decay_t<decltype(variant)>, std::reference_wrapper<int> >) {
        putVariable<int>(name, variant.get());
      } else if constexpr (std::is_same_v< std::decay_t<decltype(variant)>, std::reference_wrapper<long long int> >) {
        putVariable<long long int>(name, variant.get());
      } else if constexpr (std::is_same_v< std::decay_t<decltype(variant)>, std::reference_wrapper<float> >) {
        putVariable<float>(name, variant.get());
      } else if constexpr (std::is_same_v< std::decay_t<decltype(variant)>, std::reference_wrapper<double> >) {
  ...
  template <class T>
  void getVariable(bool registerOnly, const std::string &name, int step = -1);


  template<typename Tin, typename Tout>
  std::vector<occa::memory> getDataConvert(const std::string& name);

  template <typename T>
  std::vector<occa::memory> redistributeField(const std::vector<occa::memory>& o_in);

  template<typename T>
  void getData(const std::string& name, std::vector<occa::memory>& o_out);

  template<typename T>
  void getData(const std::string& name, variantType& variant);
