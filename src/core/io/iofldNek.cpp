...
void iofldNek::validateUserFields(const std::string& name)
{
  ...
  auto valid = false;
  if (std::find(validNames.begin(), validNames.end(), name) != validNames.end()) valid = true;
  if (std::regex_search(name, pattern)) valid = true;
...
size_t iofldNek::write()
{
  ...
  {
    nek::fldData data;
    if (auto buf = inquireVariable<double>("time")) data.time = buf->get();
    if (auto buf = inquireVariable<double>("p0th")) data.p0th = buf->get();

    if (o_x.size()) data.o_x = o_x; 
    if (auto o_buf = inquireVariable<std::vector<occa::memory>>("mesh")) data.o_x = o_buf->get();
    if (auto o_buf = inquireVariable<std::vector<occa::memory>>("velocity")) data.o_u = o_buf->get(); 
    if (auto o_buf = inquireVariable<std::vector<occa::memory>>("pressure")) data.o_p = o_buf->get(); 
    if (auto o_buf = inquireVariable<std::vector<occa::memory>>("temperature")) data.o_t = o_buf->get();
...
size_t iofldNek::read()
{
  ...
  if (auto time = inquireVariable<double>("time")) time->get() = fldData.time;
  if (auto p0th = inquireVariable<double>("p0th")) p0th->get() = fldData.p0th;

  auto populateVariable = [&](const std::string& name, const std::vector<occa::memory>& o_src)
  {
    if (platform->comm.mpiRank == 0 && platform->verbose) std::cout << " reading " << name << std::endl;
    auto& o_buf = inquireVariable<std::vector<occa::memory>>(name)->get();
    nekrsCheck(o_buf.size() != o_src.size(), MPI_COMM_SELF, EXIT_FAILURE, "invalid vector dim (%zu) of variable %s\n!", o_buf.size(), name.c_str()); 

    for (int i = 0; i < o_src.size(); i++) {
      if (o_buf[i].isInitialized()) {
         nekrsCheck(o_buf[i].size() > o_src[i].size(), MPI_COMM_SELF, EXIT_FAILURE, "invalid vector size of variable %s\n!", name.c_str());
  ...
  for (auto& entry : userFields)
  {
    ...
    if (name == "mesh" && fldData.o_x.size())  populateVariable(name, fldData.o_x); 
    if (name == "velocity" && fldData.o_u.size())  populateVariable(name, fldData.o_u); 
    if (name == "pressure" && fldData.o_p.size())  populateVariable(name, fldData.o_p); 
    if (name == "temperature" && fldData.o_t.size())  populateVariable(name, fldData.o_t); 
   
    for (int is = 0; is < fldData.o_s.size(); is++) {
      if (name == "scalar" + scalarDigitStr(is) && fldData.o_s[is].size()) populateVariable(name, fldData.o_s[is]); 
    }
  }
