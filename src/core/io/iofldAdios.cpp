...
// TODOs:
// Add element filter support for adios engine
...
void iofldAdios::openEngine()
{
    ...
    if (platform->verbose) adiosIO.SetParameters({{"verbose", "4"}});
    ...
    if (engineMode == iofld::mode::write) {
      ...
      if (step < 0) step = adiosEngine.Steps() - 1; // last
      nekrsCheck(step + 1 > adiosEngine.Steps(), platform->comm.mpiComm, EXIT_FAILURE,
                 "step number %d has to be smaller than %zu!\n", step, adiosEngine.Steps());
      if (platform->comm.mpiRank == 0) { 
        std::cout << " requested step: " << step; 
        if (step + 1 == adiosEngine.Steps()) std::cout << " (last) ";
        std::cout <<std::endl << std::flush;
      }

      for(const auto& entry : adiosIO.AvailableVariables(true)) _availableVariables.push_back(entry.first);
    }
}
...
// Converts the input to outputType, AOS, GLL or uniform grid points of a given polynomial order, 
// and finally copies the result to the host
template <typename InputType, typename OutputType>
void iofldAdios::convertField(const std::vector<occa::memory> &o_fld, occa::memory& fldConv)
{
  ...
  {
    if (!uniform && (mesh_vis->N == mesh->N)) return o_fld[dim_i];
    ...
    auto o_tmp = platform->device.malloc<dfloat>(mesh_vis->Nlocal);
    if (uniform) {
    ...
    // note, o_work is pre-allocated and resides on host
    o_work[dim_i].copyFrom(o_tmp);
    return o_work[dim_i];
  };

  auto fldConvPtr = static_cast<OutputType*>(fldConv.ptr());
  for (int n = 0; n < fldConv.size(); ++n) fldConvPtr[n] = 0;

  if (dim_fld == 1) {
    auto fld0 = map(0);
    auto fld0Ptr = static_cast<InputType*>(fld0.ptr());

    for (int n = 0; n < fld0.size(); ++n) {
      fldConvPtr[n] = static_cast<OutputType>(fld0Ptr[n]);
    }
  } else if (dim_fld == 3) {
    auto fld0 = map(0);
    auto fld0Ptr = static_cast<InputType*>(fld0.ptr());
    ...
    auto fld1 = map(1);
    auto fld1Ptr = static_cast<InputType*>(fld1.ptr());

    auto fld2 = map(2);
    auto fld2Ptr = static_cast<InputType*>(fld2.ptr());

    // VTK expects AOS
    for (int n = 0; n < fld0.size(); ++n) {
      fldConvPtr[n * 3 + 0] = static_cast<OutputType>(fld0Ptr[n]);
      fldConvPtr[n * 3 + 1] = static_cast<OutputType>(fld1Ptr[n]);
      fldConvPtr[n * 3 + 2] = static_cast<OutputType>(fld2Ptr[n]);
    }
  }
};
...
size_t iofldAdios::write_()
{
  ...
  // pre-allocate to avoid potential data relocation (caused by resize of memPool)
  auto o_connectivity = platform->memPool.reserve<uint64_t>(static_cast<uint64_t>(NumOfCells) * (mesh_vis->Nverts + 1));
  ...
  for (int i = 0; i < mesh_vis->dim; i++) {
    o_work.push_back(platform->memPool.reserve<dfloat>(mesh_vis->Nlocal));
  }
  ...
  if (getStepCounter() == 0 || platform->options.compareArgs("MOVING MESH", "TRUE")) {
    putVariable<uint32_t>("types", VTK_CELL_TYPE);
    putVariable<uint32_t>("numOfCells", NumOfCells);
    putVariable<uint32_t>("numOfPoints", NumOfPoints);
    putVariable<uint32_t>("polynomialOrder", mesh_vis->N);
    ...
    putVariable<uint64_t>("globalElementIds", o_globalElementsIds, {o_globalElementsIds.size()});

    generateConnectivity(o_connectivity);
    putVariable<uint64_t>("connectivity", o_connectivity, 
    ...
    convertField<dfloat, OutputType>(o_xyz, o_coordVertices);
    putVariable<OutputType>("vertices", o_coordVertices,
  ...
  for (const auto& entry : userFields) {
    ...
    if (inputType == occa::dtype::get<double>()) {
      convertField<double, OutputType>(o_fld, o_fldConverted);
    } else {
      convertField<dfloat, OutputType>(o_fld, o_fldConverted);
    }

    o_fldDataScratchIt++;

    const auto count = [&]() { 
      // TODO: replace mesh_vis->Nlocal as it won't work for filtered element output
  ...
  if (getStepCounter() == 0) adiosIO.DefineAttribute<std::string>("vtk.xml", vtkSchema());

  adiosEngine.PerformDataWrite(); // in case close() is not called
  adiosEngine.EndStep();
...
std::vector<occa::memory> iofldAdios::redistributeField(const std::vector<occa::memory>& o_in)
{
  ...
  const auto targetInfo = [&]()
  {
    ...
    if (o_inEntrySize == 0) return targetInfo;
  ...
  const auto maxRemoteSizes = [&]() 
  {
    ...
    MPI_Allreduce(MPI_IN_PLACE, work.data(), work.size(), MPI_INT, MPI_MAX, platform->comm.mpiComm); // ensure win is large enough on all ranks
  ...
  MPI_Win_lock_all(MPI_MODE_NOCHECK, win); // no other process holds, or will attempt to acquire, a conflicting lock,
  if (o_inEntrySize) {
...
std::vector<occa::memory> iofldAdios::getDataConvert(const std::string& name)
{
  ...
  std::vector<occa::memory> o_out; 
  if(!in.isInitialized()) return o_out;

  nekrsCheck(in.mode() != "Serial", MPI_COMM_SELF, EXIT_FAILURE, 
...
template<typename Tadios>
void iofldAdios::getData(const std::string& name, variantType& variant)
{
    if (std::holds_alternative<std::reference_wrapper<int>>(variant)) {
      auto& value = std::get<std::reference_wrapper<int>>(variant).get();
      if (platform->comm.mpiRank == 0) value = *(variables[name].data.ptr<Tadios>());
...
template <class T>
void iofldAdios::getVariable(bool allocateOnly, const std::string &name, int _step)
{
    auto adiosVariable = adiosIO.InquireVariable<T>(name);
    nekrsCheck(!static_cast<bool>(adiosVariable), MPI_COMM_SELF, EXIT_FAILURE, "variable %s not found!\n", name.c_str());
    
    if (!allocateOnly) {
      nekrsCheck(variables.count(name) == 0, MPI_COMM_SELF, EXIT_FAILURE, "variable %s was not allocated yet!\n", name.c_str());

      auto var = variables[name];
      if (!var.data.isInitialized()) return;
      ...
      if (var.dim > 0) {
        ...
        if (platform->comm.mpiRank == 0) adiosEngine.Get(name, var.data.ptr<T>(), getMode);
      }
      return;
    }
    ...
    nekrsCheck(globalBlocks.size() == 0, MPI_COMM_SELF, EXIT_FAILURE, 
    ...
    for (auto &block : globalBlocks) {
      if ((block.BlockID % static_cast<size_t>(platform->comm.mpiCommSize)) ==  static_cast<size_t>(platform->comm.mpiRank)) {
    ...
    if (var.blocks.size() == 0) {
      ...
      {
        size_t sum = 0;
        for (const auto& entry : var.blocks) sum += entry.second; 
        return sum;
      }();
    ...
    if (platform->verbose && var.blocks.size()) { 
      std::cout   << " " << name
                  << " on rank " << platform->comm.mpiRank
                  << " is of type " << var.type; 

      if(var.dim) {
        std::cout << " has " << var.blocks.size() << " out of " << globalBlocks.size() << " blocks in step " << varStep
                  << " with total entries " << var.data.size() << " and dim " << var.dim << std::endl;
      } else {
        std::cout << std::endl;
      }
    }
...
size_t iofldAdios::read()
{
  const auto userVariables = [&]() 
  {
    ...
    for(const auto& entry : userFields) variables.push_back(entry.first); 
    for(const auto& entry : userSingleValues) variables.push_back(entry.first); 
    return variables;
  }();
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    for(const auto& name : userVariables) {
      if (std::find(_availableVariables.begin(), _availableVariables.end(), name) == _availableVariables.end()) {
...
{
  if (static_cast<bool>(adiosEngine)) adiosEngine.Close();
}
