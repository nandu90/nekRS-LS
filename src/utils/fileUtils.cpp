...
void fileBcast(const fs::path &srcPathIn, const fs::path &dstPath, MPI_Comm comm, int verbose)
{
  int rank;
  ...
  if (nodeRank == nodeRankRoot) {
    ...
    nekrsCheck(!fs::exists(srcPathIn),
               MPI_COMM_SELF,
               EXIT_FAILURE,
               "Cannot find %s!\n",
  ...
  for (int i = 0; i < nFiles; i++) {
    ...
    if (nodeRank == nodeRankRoot) {
      ...
      nekrsCheck(fileSize > std::numeric_limits<int>::max(),
                 MPI_COMM_SELF,
                 EXIT_FAILURE,
                 "%s\n",
                 "File size too large!");
  ...
  nekrsCheck(fileBuf == nullptr, MPI_COMM_SELF, EXIT_FAILURE, "%s\n", "allocating file buffer failed!");
  ...
  for (const auto &file : fileList) {
    ...
    if (localRank == localRankRoot) {
      ...
      nekrsCheck(retVal, MPI_COMM_SELF, EXIT_FAILURE,
